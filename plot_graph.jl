...
filename_sol = string("Sol moyenne 30 s/",filename[1:length(filename)-4],"_sol.txt")
...
function build_graph()
    ...
    gif(Anim, string(filepath,"Sol moyenne 30 s/", filename[1:length(filename)-4],"_anim.gif"), fps = 1)
end

function plot_graphe(n, edges)
    x = [cos((i-1)*2*pi/n) for i in 1:n]
    y = [sin((i-1)*2*pi/n) for i in 1:n]
    ...
    scatter!(
         pl_new, x, y,
         marker = (:circle, 10, 0.5, "grey"),
         legend=false#:outertopleft
     )
     for i in 1:n
         annotate!(1.1*x[i], 1.1*y[i], string(i), 7)
         # for j in i+1:n
         #     if edges[i,j]==1
...
function plot_clique(sol, n, edges)
    ...
    annotate!(0,-1.2, string("Taille : ",length(sol)), 10)
    return pl_new
end
